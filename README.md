...
`alt`快捷键被浏览器占用, 禁用此项, 该键也没有被释放.

![2024-12-23 11 43 35.png](https://github.com/Kyouichirou/BiliBili_Optimizer/blob/main/images/2024-12-23%2011%2043%2035.png?raw=true)
...
主页和搜索页的`添加至稍后观看`功能被取代掉, 相关操作将被拦截, 替换为自定义的操作, 数据将被存到本地.

![2024-12-23 11 50 55.png](https://github.com/Kyouichirou/BiliBili_Optimizer/blob/main/images/2024-12-23%2011%2050%2055.png?raw=true)

![2024-12-24 11 37 31.png](https://github.com/Kyouichirou/BiliBili_Optimizer/blob/main/images/2024-12-24%2011%2037%2031.png?raw=true)

首页视频推荐将被分成三个种类

- 正常的B站推荐, 没有颜色, 假如观看过, 将被标记为: **绿色**, 悬浮标题: `[H-1], [H-2]`
- 通过观看和评分产生的相关视频: **蓝色**, 悬浮标题: `[R]`
- 稍后观看, 标记为: **红色**, 悬浮标题`[P]`

颜色较深, 则表示该视频为已经观看过.

![2024-12-23 11 49 11.png](https://github.com/Kyouichirou/BiliBili_Optimizer/blob/main/images/2024-12-23%2011%2049%2011.png?raw=true)

*上图, 红色, 表示该视频来自于稍后观看, 深色, 表示该视频已经观看过.*
...
由于`JavaScript`并没有原生支持矩阵的运算以及考虑到单线程执行, 为了方便计算和提高计算的速度, 代码中关于概率的计算部分, 进行了调整和优化, 如, 加入了针对单个字符的计算权重, 这是因为`js`原生api `Intl.Segmenter`针对中文的分词效果一般, 很多内容分解后为单个字符, 所以增加了对单个字符的计算权重, 以降低单个字符的影响. 计算的过程进行了优化, 如提前计算了部分需要反复调用的值...所以会看到的代码执行过程和在很多`python`中的`sklearn`相关代码执行存在明显差异.

### 6.2 数据的存储

为了避免**数据爆炸**的问题, 数据的存储主要分为两个部分:

**动态保存:**

设置上限是因为数据具有时效, 例如B站的热点推广信息, 拦截一段时间之后, 该视频不会再出现再推荐上, 则应该清除掉, 而不是继续占用空间.

- 拦截up, 根据拦截次数和添加时间清除掉不活跃的内容, 上限1000.
- 历史访问, 动态自动调节, 上限1000
- 手动添加关键词, 自动调节, 上限1000
- 拦截视频, 自动调节, 上限1000
